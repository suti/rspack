...
use rspack_core::{
  compare_chunks_with_graph, compare_modules_by_identifier, merge_runtime, BoxModule, Chunk,
...
fn get_module_size(module_id: &ModuleIdentifier, compilation: &Compilation) -> f64 {
  let mg = compilation.get_module_graph();
  mg.module_by_identifier(module_id).map_or(0f64, |module| {
    let st = module.source_types().first();
    module.size(st, Some(compilation))
  })
}

fn get_chunk_modules_size(module_ids: &IdentifierSet, compilation: &Compilation) -> f64 {
  module_ids.iter().fold(0f64, |acc, module_id| {
    get_module_size(module_id, compilation) + acc
  })
}

fn get_chunk_size(chunk_id: &ChunkUkey, compilation: &Compilation) -> f64 {
  get_chunk_modules_size(
    compilation
      .chunk_graph
...
impl ChunkMutation {
  ...
  fn optimize_stage_module_chunks(
    ...
    stage_chunks
      ...
      .filter_map(|chunks| {
        ...
        chunks.retain(|c| !entries.contains(c) && !keep_chunks.contains(&c));

        let integrate_map: UkeyMap<ChunkUkey, UkeySet<ChunkUkey>> = chunks
          .iter()
          .map(|&chunk_a| {
            (
              chunk_a,
              chunks
                ...
                .filter(|&&chunk_b| {
                  chunk_a != chunk_b
                    && Self::can_chunks_be_integrated(&chunk_a, &chunk_b, compilation)
                })
                .cloned()
                .collect::<UkeySet<ChunkUkey>>(),
            )
        ...
        let mut integrate_vec = integrate_map
          .iter()
          .map(|(k, v)| (*k, v.clone()))
          .collect::<Vec<_>>();

        integrate_vec.sort_by(|a, b| a.1.len().cmp(&b.1.len()));

        let mut groups: Vec<(ChunkUkey, UkeySet<ChunkUkey>)> = vec![];
        loop {
          let target = integrate_vec.pop();
          if let Some(target) = target {
            integrate_vec.retain(|(k, _)| !target.1.contains(k));
            integrate_vec.iter_mut().for_each(|(k, v)| {
              v.retain(|c| !target.1.contains(c));
            });
            groups.push(target);
          } else {
            break;
        ...
        groups.iter().for_each(|(root, group)| {
          let mut group = group.iter().cloned().collect::<Vec<_>>();
          let mut target = group.pop();
          let mut removed = vec![];
          loop {
            let Some(current_target) = target else {
              break;
            };
            self.integrate_chunks(*root, current_target, compilation);
            removed.push(current_target);
            target = group.pop();
          }
          chunks.retain(|&chunk| !removed.contains(&chunk));
          removed_chunk_count += removed.len();
        });
  ...
  fn concat_chunks(
    ...
    let mut small_chunks = chunk_ref
      ...
      .filter_map(|cd| {
        if !cd.is_empty()
          && get_chunk_size(&cd.chunk_key, compilation) < size_limit.0
          && !contains_shared_module(compilation, &cd.chunk_key)
          && !keep_chunks.contains(&cd.chunk_key)
        {
          Some(cd.chunk_key)
        } else {
          None
    ...
    loop {
      ...
      let mut can_be_integrated_withs = can_be_integrated_withs
        ...
        .filter(|chunk| {
          small_chunk_key != *chunk
            && !keep_chunks.contains(chunk)
            && compilation
              .chunk_by_ukey
              .get(chunk)
              .map_or(false, |chunk| chunk.name().is_none())
            && self.chunks.get(chunk).map_or(false, |chunk| {
              get_chunk_size(&chunk.chunk_key, compilation) < size_limit.1
            })
        ...
        for chunk_key in can_be_integrated_withs.iter().cloned() {
          ...
          if let Some(target) = target {
            if target != small_chunk_key {
              ...
              if get_chunk_size(&chunk.chunk_key, &compilation) < size_limit.1
      ...
      if get_chunk_size(&best_chunk_key, &compilation) > size_limit.0 {
      ...
      if let Some(small_chunk_key) = chunks.pop() {
        {
          ...
          if get_chunk_size(&integrated_chunk_key, &compilation) > size_limit.0 {
          ...
          if get_chunk_size(&small_chunk_key, &compilation) > size_limit.0 {
            integrated_chunk_key = small_chunk_key;
            continue;
          }
        }
  ...
  fn split_chunks(
    ...
    let chunks = compilation
      ...
      .collect::<UkeyMap<ChunkUkey, IdentifierSet>>();

    let split_modules = |module_ids: &IdentifierSet, size: f64, compilation: &Compilation| {
      let mut modules: Vec<IdentifierSet> = Default::default();
      module_ids.iter().fold(0f64, |acc, module_id| {
        let next = get_module_size(module_id, compilation) + acc;
        if next > size {
          modules.push(Default::default());
          modules.last_mut().map(|mods| {
            mods.insert(*module_id);
          });
          return next - acc;
        }
        if let Some(mods) = modules.last_mut() {
          mods.insert(*module_id);
        }
        next
      });
    ...
    let big_chunks = chunks
      .iter()
      .filter(|(chunk_key, module_ids)| {
        get_chunk_modules_size(module_ids, compilation) > size_limit.1 as f64
      })
      .collect::<Vec<_>>();

    // let logger = compilation.get_logger("rspack_plugin_split_chunks");
    // logger.info(format!("split_big_chunks {:?}", &big_chunks.iter().map(|k| {
    //   compilation.chunk_by_ukey.get(k.0).map(|chunk| {
    //     let name = chunk.name();
    //     chunk
    //   })
    // })));
    big_chunks.into_iter().for_each(|(chunk_key, module_ids)| {
      let mut new_chunks_with_modules = split_modules(module_ids, size_limit.0 as f64, compilation);
      new_chunks_with_modules.retain(|module_ids| module_ids.len() > 0);
      if new_chunks_with_modules.len() > 0 {
        new_chunks_with_modules.iter().for_each(|module_ids| {
...
impl SplitChunksPlugin {
  pub(in crate::plugin) fn better_chunks(
    ...
    compilation: &mut Compilation,
    delimiter: &str,
    mut keep_chunks: UkeySet<ChunkUkey>,
    ...
    if !options.keep_named_cache_group {
      keep_chunks.clear();
    }
    ...
    if log {
      ...
      logger.info(format!(
        ...
        &stage_modules
          ...
          .flatten()
          .filter_map(|id| {
            let module = compilation.module_by_identifier(id)?;
            Some(module.name_for_condition()?.to_string())
          })
          .collect::<Vec<_>>()
      ));
      logger.info(format!(
    ...
    if options.remove_duplicate_modules {
      ...
      let optimize_result =
        chunk_mutation.optimize_stage_module_chunks(compilation, stage_chunks, &keep_chunks);
    ...
    if options.concat_small_chunks {
      ...
      let mut skip_concat_chunks = options
        .skip_concat_chunks
        .iter()
        .map(|name| {
          compilation
            .named_chunks
            .iter()
            .filter(|(n, _)| n.contains(&name.clone()))
            .map(|(_, chunk)| *chunk)
            .chain(
              compilation
                .named_chunk_groups
        ...
        .flatten()
        .collect::<UkeySet<ChunkUkey>>();
      skip_concat_chunks.extend(&keep_chunks);
