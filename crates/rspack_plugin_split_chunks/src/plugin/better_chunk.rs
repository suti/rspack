...
impl ChunkMutation {
  ...
  fn find_relink_point(&mut self, compilation: &Compilation, end_points: UkeySet<ChunkUkey>) {
  ...
  fn remove_duplicate_modules(&mut self, compilation: &mut Compilation) {
    ...
    let mut chunk_modules_map: UkeyMap<ChunkUkey, IdentifierSet> = Default::default();
  ...
  fn find_best_concatenate(
    ...
    let mut small_loop_chunks_with_group_path: Vec<(ChunkUkey, usize)> = Default::default();
    let mut small_chunks_with_group_path = chunk_ref
      .values()
      .filter(|cd| !cd.is_empty())
      .filter_map(|cd| {
        cd.chunk_size(&self.chunks, &self.modules)
          ...
          .and_then(|chunk_key| {
            ...
            let chunk = compilation.chunk_by_ukey.expect_get(&chunk_key);
            let mut traversed_group = UkeySet::default();
            let mut can_integrate_to = true;
            let mut parent_paths = chunk
              .groups()
              .iter()
              .map(|&cgk| {
                ChunkGroupPath::find_parent_chunk_group_path(
                  cgk,
                  end_points.clone(),
                  compilation,
                  &mut traversed_group,
              ...
              .flatten()
              .filter_map(|path| {
                // if path.end_with_end_point {
                //   Some(path.chunk_group_paths)
                // } else {
                //   can_integrate_to = false;
                //   None
                // }
                Some(path.chunk_group_paths)
              }) // 去除循环
              .collect::<Vec<_>>();
            // 存在chunkgroup循环引用且存在Runtime或者Entry的chunk不能合并到其他chunk
            if parent_paths.is_empty() {
              return None;
            }
            parent_paths.sort_by(|a, b| a.len().cmp(&b.len()));
            let len = parent_paths.first().map_or(0, Vec::len);
            if !can_integrate_to {
              small_loop_chunks_with_group_path.push((chunk_key, len));
              return None;
            }
            Some((chunk_key, parent_paths, len))
          })
      })
      .collect::<Vec<_>>();

    small_chunks_with_group_path.sort_by(|a, b| b.2.cmp(&a.2));

    let mut small_chunks = small_chunks_with_group_path
      .into_iter()
      .map(|(chunk_key, paths, parent_paths)| chunk_key)
      .collect::<Vec<_>>();
    ...
    loop {
      ...
      let can_be_integrated_withs = small_chunk
        ...
        .map(|&group| {
          compilation
            .chunk_group_by_ukey
            .get(&group)
            .map(|group| vec![group.chunks.clone()])
        })
        .flatten()
        .flatten()
        .flatten()
      ...
      let mut can_be_integrated_withs = can_be_integrated_withs
        ...
        .collect::<Vec<_>>();
      can_be_integrated_withs
        .sort_by(|a, b| compare_chunks_with_graph(chunk_graph, &module_graph, a, b));
      ...
      let small_chunk_appended_chunk = {
        ...
        std::mem::take(&mut small_chunk.appended_chunk)
      };
      {
        ...
        {
          panic!("best chunk size is bigger than limit {:?}", best_chunk);
        }
      }
      {
    ...
    not_found_best_chunk.sort_by(|a, b| {
      let a_chunk = compilation.chunk_by_ukey.get(a).expect("must in");
      let b_chunk = compilation.chunk_by_ukey.get(b).expect("must in");
      a_chunk
        .get_all_referenced_chunks(&compilation.chunk_group_by_ukey)
        .len()
        .cmp(
    ...
    let mut not_found_best_chunk_1: Vec<ChunkUkey> = vec![];
    ...
    loop {
      if let Some(small_chunk_key) = not_found_best_chunk.pop() {
        ...
        if Self::can_chunks_be_integrated(&integrated_chunk_key, &small_chunk_key, &compilation)? {
          ...
          if second_loop {
            not_found_best_chunk_1.push(small_chunk_key);
          }
        }
      } else {
        if second_loop {
          not_found_best_chunk_1.sort_by(|a, b| {
            let a_size = self
              .chunks
              .get(a)
              .map_or(SplitChunkSizes::empty(), |chunk| {
                chunk.chunk_size(&self.chunks, &self.modules)
              });
            let b_size = self
              .chunks
              .get(b)
            ...
            a_size.total_size().cmp(&b_size.total_size())
          });
          not_found_best_chunk = mem::take(&mut not_found_best_chunk_1);
          second_loop = false;
          continue;
        }
        break;
  ...
  fn concat_chunks(&self, compilation: &mut Compilation) {
    let mut removed_chunks: UkeySet<ChunkUkey> = Default::default();
    ...
    self.chunks.values().for_each(|chunk| {
      if removed_chunks.contains(&chunk.chunk_key) || chunk.removed {
        return;
      }
      chunk.appended_chunk.iter().for_each(|&appended_chunk_id| {
        removed_chunks.insert(appended_chunk_id);
        ...
        chunk_by_ukey.remove(&appended_chunk_id);
        chunk_graph.remove_chunk(&appended_chunk_id);
      });
    });

    if let Some(mutations) = compilation.incremental.mutations_write() {
      self.chunks.values().for_each(|chunk| {
        if removed_chunks.contains(&chunk.chunk_key) {
          mutations.add(Mutation::ChunkRemove {
            chunk: chunk.chunk_key,
          });
        } else if chunk.appended_chunk.len() > 0 {
          mutations.add(Mutation::ChunksIntegrate {
            to: chunk.chunk_key,
          });
        }
      });
...
impl SplitChunksPlugin {
  pub(in crate::plugin) fn better_chunks(
    ...
    logger.info(format!("size_limit {:?}", &size_limit));
    ...
    chunk_mutation.find_relink_point(compilation_ref, end_points.clone());

    // logger.info(format!(
    //   "chunk_parent_chunk_group_paths_map {:#?}",
    //   &chunk_mutation.chunk_parent_chunk_group_paths_map
    // ));

    // logger.info(format!("chunk_mutation {:?}", &chunk_mutation));
    // logger.info(format!("entry_points {:?}", &compilation_ref.entrypoints));
    // logger.info(format!(
    //   "entry_points {:?}",
    //   &compilation_ref
    //     .entrypoints
    ...
    chunk_mutation.remove_duplicate_modules(compilation);

    chunk_mutation.remove_empty_chunks(compilation);

    chunk_mutation.find_best_concatenate(&size_limit, compilation, end_points);

    chunk_mutation.concat_chunks(compilation);

    // chunk_mutation.split_chunks(&size_limit, compilation);

    logger.info(format!("entry_points {:?}", &compilation.entrypoints));
    logger.info(format!(
      "entry_points {:?}",
      &compilation
        .entrypoints
